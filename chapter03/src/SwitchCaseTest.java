 ...
 * 执行完此执行语句之后，
 *    情况1：遇到break，则执行break后，跳出当前switch-case结构
 *    情况2：没有遇到break，则继续执行其后的case中的执行语句。 ---> case 穿透
 *          。。。
 *          直到遇到break或者执行完所有的case及default中的语句，然后结束当前的switch-case结构
 *
 * 3.说明：
 * ① switch中的表达式只能是特定的数据类型，如：byte/short/char/int/枚举（JDK5.0）/String（JDK7.0）
 * ② case 后都是跟的常量，使用表达式与这些常量做相等的判断，不能进行范围的判断
 * ③ 开发中，使用switch-case时，通常case匹配的情况都有限
 * ④ break：可以使用在switch-case中，一旦执行此break关键字，就跳出此siwtch-case结构
